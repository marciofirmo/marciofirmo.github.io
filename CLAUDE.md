...
- const SPL = {...}: link de playlist pública do Spotify por PAÍS, chave "SIGLA". Usado como fallback (1 botão por país) quando o jogo não tem entrada em PL. Preferir sempre playlists públicas reais (OWNER/oficiais) — as geradas pelo conector nascem privadas e não abrem para visitantes.
